...
class NTS_1m_Env:
    ...
    def data_pre(self, data_name,minute):
        ...
        db.execute("SELECT close FROM public.price_omin WHERE symbol="+data_name+" ORDER BY datetime ASC, Symbol ASC;")
        dt = db.fetchall()
        dt = self.time_Frame(dt, minute)

        db.execute("SELECT datetime FROM public.price_omin WHERE symbol="+data_name+" ORDER BY datetime ASC, Symbol ASC;")
        date = db.fetchall()
