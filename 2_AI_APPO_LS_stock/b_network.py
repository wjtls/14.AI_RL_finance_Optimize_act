...
class Global_critic(nn.Module):
    def __init__(self, device, window,dim,save_name,Neural_net, bidirectional_):
        ...
        # 현재 실행 중인 스크립트의 디렉토리를 기준으로 체크포인트 경로 생성
        self.check_point = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'future_Global_critic_' + save_name)
...
class Global_actor(nn.Module):
    def __init__(self, device, window,dim,save_name,Neural_net, bidirectional_):
        ...
        # 현재 실행 중인 스크립트의 디렉토리를 기준으로 체크포인트 경로 생성
        self.check_point = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'future_Global_actor_' + save_name)
