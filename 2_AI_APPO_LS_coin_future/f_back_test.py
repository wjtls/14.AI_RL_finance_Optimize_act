...
class back_testing:
    def __init__(self,train_val_test):
        ...
        # 트레이딩시 초기 날짜
        days_ago = pd.read_excel('trading_times.xlsx')
...
if __name__=='__main__':
    ...
    # npy 저장 불러오기
    csv_agent_name= 'z_result.csvlong_4'
    ...
    '''''
    # npy 저장 불러오기
    npy_agent_name= 'z_result.npylong_4.npy'
    loaded_data = np.load(npy_agent_name, allow_pickle=True).item()
    cumul_epi_reward = loaded_data['self.epi_reward_data']
    cumul_reward = loaded_data['self.cumul_reward_data']
    cumul_policy_loss = loaded_data['self.policy_loss_data']
    cumul_value_loss = loaded_data['self.value_loss_data']
    cumul_PV = loaded_data['self.cumul_PV_data']
